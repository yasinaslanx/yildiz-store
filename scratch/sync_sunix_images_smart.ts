...
async function main() {
  ...
  for (const dbProduct of missingProducts) {
    ...
    // 2. Token overlap (if 2 or more important words match)
    if (!match) {
        let bestMatch = null;
        let maxOverlap = 0;
    ...
    if (match) {
      console.log(`[MATCH] DB: "${dbProduct.name}" <-> Portal: "${match.name}" (Code: ${dbCode})`);
      matchCount++;
    } else {
      console.log(`[NO MATCH] DB: "${dbProduct.name}" (Code: ${dbCode})`);
    }
  }

  console.log(`\nSmart matched: ${matchCount} out of ${missingProducts.length}`);
}
