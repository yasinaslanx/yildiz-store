...
export function QuickViewModal({ isOpen, onClose, product }: QuickViewModalProps) {
  ...
  const finalPrice = selectedVariant ? (isDealer && (selectedVariant as any).dealerPrice ? Number((selectedVariant as any).dealerPrice) : Number(selectedVariant.price)) : Number(product.price || 0);
